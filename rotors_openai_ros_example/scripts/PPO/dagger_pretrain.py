...
"""
def ppo(env_fn, ref_func =None, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=500, epochs=10000, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
    ...
    for epoch in range(1, epochs + 1, 1):
        for t in range(local_steps_per_epoch):
            a_s, v_t, logp_t = sess.run(get_action_ops, feed_dict={x_ph: np.array(o).reshape(1,-1)})
            a = a_s[0]
...
if __name__ == '__main__':
    rospy.init_node('pelican_attitude_controller_dagger_pretrain', anonymous=True, log_level=rospy.WARN)
    import argparse
    ...
    parser.add_argument('--exp_name', type=str, default='dagger_for_ppo')
    ...
    ppo(lambda : env, ref_func = call_nlmpc, actor_critic=core.mlp_actor_critic,
