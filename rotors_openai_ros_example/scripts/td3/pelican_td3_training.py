...
def td3(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        ...
        polyak=0.995, pi_lr=1e-4, q_lr=1e-4, batch_size=64, start_epochs=1000, 
        act_noise=0.1, target_noise=0.2, noise_clip=0.5, policy_delay=2, 
        max_ep_len=500, logger_kwargs=dict(), save_freq=100, UPDATE_STEP=10):
    """
    ...
    def test_agent(n=10, test_num=1):
        for j in range(n):
            o, r, d, ep_ret, ep_len = test_env.reset(), 0, False, 0, 0
            while not(d or (ep_len == max_ep_len)):
                # Take deterministic actions at test time (noise_scale=0)
                o, r, d, _ = test_env.step(get_action(np.array(o), 0))
                ep_ret += r
                ep_len += 1
